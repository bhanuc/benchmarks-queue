...
sudo apt-get update       
sudo apt-get install -y build-essential pkg-config htop postgresql postgresql-contrib
...
node_versions='10.24.1 12.22.1 14.16.1 16.0.0 node'
...
for pg_version in $pg_versions; do
    ...
    for c in 1 5 20 50; do
    for i in 0 1 2; do
        echo "$name@$c [$node_version #$i] {pg $pg_version}"
        # run redis-server in the background, ignore its output, and disable
        # persist-to-disk.
